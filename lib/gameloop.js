...
import { doEncode, doDecode} from "./encodetilemap.js"
...
const client = createClient({
    password: 'dro7XWIKylgDU', // @todo DANGER use env and send as privet to fleek
    socket: {
        host: 'redis-17473.c99.us-east-1-4.ec2.redns.redis-cloud.com',
        port: 17473
    }
});
...
export async function render(userId, action) {
    //if(!connected) {    
    ...
    */
    ...
    function populateRoom(room, dungeonLevel) {
        ...
        const itemChances = {
            'healing potion': 70,
            'lightning scroll': 0,// evaluateStepFunction([[4, 25]], dungeonLevel),
            'fireball scroll':  0,//evaluateStepFunction([[6, 25]], dungeonLevel),
            'confusion scroll':  0,//evaluateStepFunction([[2, 10]], dungeonLevel),
            sword:  0,//evaluateStepFunction([[4, 5]], dungeonLevel),
            shield:  0,//evaluateStepFunction([[8, 15]], dungeonLevel),
        };
    ...
    function serializeGlobalState() {
        const _tileMap = JSON.parse(JSON.stringify(tileMap));
        //delete _tileMap.fov;
        _tileMap._values = doEncode(_tileMap._values);
        //console.log(_tileMap._values, "before")
        ...
        //console.log(JSON.stringify(compress(JSON.parse(JSON.stringify(saved)))))
        return JSON.stringify((saved));
    }

    function deserializeGlobalState(json) {
        //console.log(json.length, "lengtttt")

            // lets uncompress
            json = Buffer.from(json, 'base64');
            //console.log("base64", json)
            // decompress buffer
            json = zlib.inflateSync(json).toString();
            //console.log("decompressed", json)
            // convert to string
//console.log(json)
            // convert to unpack json
        const saved = (JSON.parse(json));
        
        saved.tileMap._values = doDecode(saved.tileMap._values);
        ...
        //console.log(saved.tileMap._values)
        tileMap._values = saved.tileMap._values
        //console.log("afteer", JSON.stringify(saved.tileMap._values))
    ...
    */
    ...
    async function runAction(action) {
        ...
        case 'restore-game': {
            // this is a bse64 encoded string compressed and packed
            //let json = db.gameplays[userId] || null;
            let json = db.state || null;
            //json = await client.get('state.'+userId);
        ...
        case 'save-game': {
            let jsonStr = serializeGlobalState();
            ...
            //console.log(typeof jsonpack.pack(json));
            //const unpacked = jsonpack.unpack(packed);
            //console.log("unpacked", JSON.stringify(unpacked))
            //console.log(jsonpack.unpack(jsonpack.pack(json)))
            // @todo: this is disabled for now
            //const compressedBuffer = zlib.brotliCompressSync(Buffer.from(jsonpack.pack(json)));
            //console.log(jsonStr)
            const compressedBuffer = zlib.deflateSync(Buffer.from(jsonStr));
            //db.gameplays[userId] = compressedBuffer.toString('base64');
            db.state = compressedBuffer.toString('base64');
            //.set('state.'+userId, db.state);
            //await db.write();
    ...
    if(player.hp <=0) {
        return 'https://ipfs.io/ipfs/QmSPhMn4rbkT6D328UyLyXGiX83KS6fZPX8g4Dk8Jvgf9j';
    }
    ...
    try {
        let item = itemEntityAt(player.location.x, player.location.y);
        if(item && item.type == 'healing potion'){
            moveEntityTo(item, {carried_by: player.id, slot:0});
            useItem(player, item)
        }
        let onStairs = allEntitiesAt(player.location.x, player.location.y).filter(e => e.stairs);
        if (onStairs.length > 0) {
            await runAction(['stairs-down']);
        }
    } catch(err ){}
    ...
    let svgString = display.render(player.location.x,player.location.y);
/*

...
export default class Svg {
	...
	render(x, y) {
        ...
        // zoom, use x,y to center the camera
        const zoom = {
        dx: Math.floor((-x) * (this.opts.fontSize +6)),
        dy: Math.floor((-y) * (this.opts.fontSize + 6))
        }
        svg += `<g transform="translate(${zoom.dx} ${zoom.dy}) scale(1.9)">`;
        //svg += `<g transform="scale(1)">`;
        //   <g transform="translate(1080 1100) scale(1.2)">
		...
		if (this.maxHp) {
			...
			// life bar
			svg += `<rect x="50" y="${(this.opts.height)*(this.opts.fontSize + 4)}" style="fill:${color};stroke:black;stroke-width:5px" width="${(this.opts.width*this.opts.fontSize* pct) / 100}" height="100px"></rect>`;
		...
		this.messages.reverse().forEach((message, i) => {
			const font = `font-family="${this.opts.fontFamily}" font-size="${Math.ceil(this.opts.fontSize * 1.4)}" fill="white"`;
			svg += `<text ${font} x="50" y="${(this.opts.height + 4)*(this.opts.fontSize + 5) + (i*(this.opts.fontSize + 6))}">${message}</text>`;
